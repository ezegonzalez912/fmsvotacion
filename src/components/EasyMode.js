...
export const EasyMode = ({setEasyMode, easyMode, competidor, cambiarMC, setCambiarMC, setCompetidor, ocultarTotal}) => {  
  ...
  useEffect(() => {
    setEasyMode(funcionInput(input1) + funcionInput(input2) + funcionInput(input3) + funcionInput(input4) 
    + funcionInput(input5) + funcionInput(input6) + funcionInput(input7) + funcionInput(input8) + funcionInput(input9))
  }, [input1, input2, input3, input4, input5, input6, input7, input8, input9, setEasyMode])
  ...
  return (
    <Contenedor>
      <div className='title-contenedor'>
        {
            ...
            cambiarMC ? <><input value={competidor} maxLength={8} onChange={ e => setCompetidor(e.target.value)}/>
            <img src={save} onClick={() => setCambiarMC( (estado) => !estado)}/>
          </>
          : <> 
              <h2 className='title-competidor'>{competidor}</h2>
              <img src={edit} onClick={() => setCambiarMC( (estado) => !estado)}/>
            </> 
        }
      ...
      <div className='inputs-contenedor'>
        ...
        {ocultarTotal ? <input tabIndex='-1' className='input input-resultado' readOnly value={easyMode}/> : <input tabIndex='-1' className='input input-resultado' readOnly value='-'/>}
      </div>
    </Contenedor>
  )
}
